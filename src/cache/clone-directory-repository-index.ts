...
let cloneIndexMemoryCache: RepositoryIndexSnapshot | undefined;
...
function isValidSnapshot(snapshot: RepositoryIndexSnapshot | undefined, cloneDirectory: string) {
...
function readStoredCloneIndexSnapshot(cachePath: string) {
  return readJsonFile<RepositoryIndexSnapshot | undefined>(cachePath, undefined);
}

function readRepositoryIndexSnapshot(cloneDirectory: string, cachePath: string) {
...
function writeRepositoryIndexSnapshot(cachePath: string, cloneDirectory: string, entries: RepositoryIndexEntry[]) {
...
async function indexCloneDirectoryRepositories(
  cloneDirectory: string,
  cachePath?: string,
  scannedRepositories?: RepositoryIndexEntry[],
  ...
  if (discoveredRepositories.length) {
    ...
    if (cachePath) {
      writeRepositoryIndexSnapshot(cachePath, cloneDirectory, entries);
    }
...
function cloneDirectoryRepositoryIndexFromEntries(entries: RepositoryIndexEntry[]): CloneDirectoryRepositoryIndex {
  const worktreeByRepositoryKey = new Map<string, string>();
  const remoteUrlByWorktree = new Map<string, string>();
...
export async function prepareCloneDirectoryIndex(
  cloneDirectory: string,
  options?: { force?: boolean; cachePath?: string; scannedRepositories?: RepositoryIndexEntry[] },
): Promise<CloneDirectoryRepositoryIndex> {
  if (!options?.force && options?.cachePath) {
    const snapshot = readRepositoryIndexSnapshot(cloneDirectory, options.cachePath);
    if (snapshot) {
